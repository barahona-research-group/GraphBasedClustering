...
%     dataNcut.verbose = 0; 0 for verbose off mode, 1,2,3 for verbose on modeseigs
...
P = spmtimesd(W,Dinvsqrt,Dinvsqrt); %D^(-1/2)*W*D^(-1/2)
clear W;
...
[vbar,s,convergence] = eigs(@(x) mex_w_times_x_symmetric(x,tril(P)),size(P,1),nbEigenValues,'LA',options); 
%warning on

s = real(diag(s)); %the eigenvalues
[x,y] = sort(-s);  %sort the eigenvalues
Eigenvalues = -x;  %top eigenvalues
vbar = vbar(:,y);  %top eigenvectors
Eigenvectors = spdiags(Dinvsqrt,0,n,n) * vbar; % D^(-1/2) * eigenvector
