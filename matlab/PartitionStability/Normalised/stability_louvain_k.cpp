...
bool parse_arg(int nrhs, const mxArray *prhs[]) {
	...
	if (nrhs > 1) {
		num_community = ((int) mxGetScalar(prhs[1]));
	}
	if (nrhs > 2) {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	if (parse_arg(nrhs, prhs) && nlhs < 4) {
		...
		while (numberofcommunities > 2) {
			...
			if (numberofcommunities == num_community)
				{break;}
		...
		printf("\n\n\t[stability]=stability_louvain_LNL(graph)");
		printf("\n\t[stability]=stability_louvain_LNL(graph, markov time)");
		printf(
				"\n\t[stability]=stability_louvain_LNL(graph,  markov time, precision)");
		printf(
				"\n\t[stability]=stability_louvain_LNL(graph,  markov time, precision, weighted)");
		printf(
				"\n\t[stability]=stability_louvain_LNL(graph,  markov time, precision, weighted, hierarchy)");
		printf(
				"\n\t[stability, number of communities]=stability_louvain_LNL(graph)");
		printf(
				"\n\t[stability, number of communities]=stability_louvain_LNL(graph, markov time)");
		printf(
				"\n\t[stability, number of communities]=stability_louvain_LNL(graph,  markov time, precision)");
		printf(
				"\n\t[stability, number of communities]=stability_louvain_LNL(graph,  markov time, precision, weighted)");
		printf(
				"\n\t[stability, number of communities]=stability_louvain_LNL(graph,  markov time, precision, weighted, hierarchy)");
		printf(
				"\n\t[stability, number of communities, communities]=stability_louvain_LNL(graph)");
		printf(
				"\n\t[stability, number of communities, communities]=stability_louvain_LNL(graph, markov time)");
		printf(
				"\n\t[stability, number of communities, communities]=stability_louvain_LNL(graph,  markov time, precision)");
		printf(
				"\n\t[stability, number of communities, communities]=stability_louvain_LNL(graph,  markov time, precision, weighted)");
		printf(
				"\n\t[stability, number of communities, communities]=stability_louvain_LNL(graph,  markov time, precision, weighted, hierarchy)");
