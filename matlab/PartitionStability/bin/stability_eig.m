function [S, N, VI, C, Call] = stability_eig(G, T, varargin)
...
[U,Lambda] = eig(full(Graph),full(diag(sum(Graph))));
Lambda = diag(Lambda);
...
for t=1:length(Time)
    ...
    [S(t), N(t), C(:,t), VI(t), Call1] = StabilityFunction(Graph,U,Lambda, Time(t), Precision, weighted, ComputeVI, NbLouvain, M, NbNodes, ComputeParallel);
...
%------------------------------------------------------------------------------
function [S, N, C, VI, Call] = louvain_FNL(Graph, U, Lambda, time, precision, weighted, ComputeVI, NbLouvain, M, NbNodes, ComputeParallel)
...
clear trans;
% tic
exponential = sparse(U * diag(exp((Lambda-1).*time)) * U'*D);
% toc
% tic
% exponential1=sparse(expm(full(time.*Lap))); %%
% toc
clear Lap;
