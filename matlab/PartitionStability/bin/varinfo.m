...
%   of partitions where M is the number of nodes in the original graph
%   and N is the number of partitions. The output VI is the average variation
...
%   the partitions contained in row i and j of the matrix P(Npartition by Nnodes).
%   
...
if nargin==2 && ComputeParallel
    ...
    for i = 1:number_of_partitions
        partition_1 = partition_vectors(i,:);
        partition_1 = double(partition_1)+1;
        A_1 = sparse(partition_1,nodes,1); % indicator matrix, Ncluster by Nnodes  
        n_1_all = sum(A_1,2); %number of nodes in each cluster
